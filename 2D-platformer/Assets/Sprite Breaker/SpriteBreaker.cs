...
public class SpriteBreaker : MonoBehaviour
{
    ...
    [Tooltip("Gravity for fake physics")]
    [Description("Gravity")]
    ...
    [Tooltip("Modify Gravity scale for custom gravity.")]
    public float customGravityScale = 10.0f;
    ...
    [Description("±")]
    [Tooltip("Plus or minus randomness % variation for radial impulse")]
    [Range(0, 100)]
    ...
    [Description("±")]
    [Tooltip("Plus or minus randomness % multiplier for linear impulse")]
    [Range(0, 200)]
    ...
    [Description("±")]
    [Tooltip("Plus or minus randomness % multiplier for rotational impulse")]
    [Range(0, 100)]
    public float initialRotationalImpulsePlusMinus = 10f;

    [Description("Time to live (seconds)")]
    [Tooltip("If set to non-0, the shards will be destroyed after this time interval after Break() is called.")]
    [Min(0)]
    public float timeToLive = 2f;

    [Tooltip("If timeToLive is set, the shards will fade to alpha=0 after this time.")]
    [Min(0)]
    ...
    private GameObject SpawnShard(int polyIndex, Transform parent, Sprite baseSprite, SpriteRenderer baseRenderer, Rect uvTransform)
    {
        ...
        if (poly.includeInPhysics)
        {
            ...
            if (createColliders == CreateColliders.Collider2D)
            {
                ...
                body.gravityScale = customGravityScale;
