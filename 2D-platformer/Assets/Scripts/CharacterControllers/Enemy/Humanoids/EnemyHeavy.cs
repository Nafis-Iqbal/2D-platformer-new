...
public class EnemyHeavy : EnemyBase
{
    ...
    public float minimumBlockingDistance, maximumBlockTime, minimumTimeBetweenBlocks;
    float lastBlockEndTime, lastBlockStartTime;
    ...
    public override void HandleAttackPlayerAnimation()
    {
        ...
        if (isDefensive && !isCharacterBlocking)
        {
            if (playerDistanceFromEnemy < minimumBlockingDistance && Time.time - lastBlockEndTime > minimumTimeBetweenBlocks &&
            characterHealthStaminaScript.staminaCompletelyDepleted == false && characterHealthStaminaScript.poiseBroken == false)
            {
        ...
        else if (isCharacterBlocking && (playerDistanceFromEnemy > minimumBlockingDistance || Time.time - lastBlockStartTime > maximumBlockTime || characterHealthStaminaScript.staminaCompletelyDepleted == true ||
         characterHealthStaminaScript.poiseBroken == true))
        {
